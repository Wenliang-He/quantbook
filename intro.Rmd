...
| Class | Variable Type | Treatment | Examples |
|:---|:---|:---|:---|
| categorical | dummy | dummy | gender |
| categorical | nominal | dummy | city |
| categorical | ordinal | nominal or continuous|education level |
| continuous | interval | continuous | temperature in Celsius |
| continuous | ratio | continuous | temperature in Kelvin |
...
Notice that the mean suddenly increases from sample 1 to sample 2.
...
The medians, however, remain relatively stable across samples. 
...
> `QUESTION`: How do you interpret median? For example, how do you interpret the statement that "the median annual household income is $50,000 in Smallville"?
...
$$s = \sqrt{v}$$
where $v$ stands for **variance** and $s$ for **standard deviation**. 

> `QUESTION`: Is variance sensitive to outliers?

> `QUESTION`: Why do we compute the average of $(x_i-m)^2$ instead of $(x_i-m)$. 

If the average of $x_i$ is the mean, then what are the aveages of $(x_i-m)$ and $(x_i-m)^2$?
...
> `QUESTION`: In data analysis reports, we will use standard deviation much more frequently than variance. Why is this the case?
...
- The **_first quartile_** is also called the **_25th percentile_**
- The **_second quartile_** is also called the **_50th percentile_** (a.k.a. median)
- The **_third quartile_** is also called the **_75th percentile_**
...
> `QUESTION`: Are statistics from the five-number summary susceptible to outliers? 
...
- We have some ideas about the magnitude of SDs.
