...
One problem with _difference in proportions_ is that it does not handle small proportions well. For example, is 0.05 a small or big difference? It would be considered small if it's between 0.95 and 0.90; but would be a big difference if it's between 0.10 and 0.15.
...
In the 2-by-2 contingency table, the null hypothesis $H_0: p_T = p_C$ is equivalent to the claim that X and Y are independent. 

`Proof`:

By definition, we have $p_C = P(Y=1|X=0)$ and $p_T = P(Y=1|X=1)$. Under the null hypothesis $p_C = p_T$, we have $P(Y=1|X=0) = P(Y=1|X=1)$, which implies
...
Given a 2-by-2 table, i.e., I=2 and J=2, knowning the sample size n, 

> How many cells can be freely decided under the null hypothesis?

**Answer**: 2 = (2-1) + (2-1) 

> How many cells can be freely decided under the alternative hypothesis?
