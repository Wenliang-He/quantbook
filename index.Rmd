...
Install **Git for Windows**, aka `msysgit` or "Git Bash", which installs Git along with other useful tools, such as the Bash shell. 

* Go to [gitforwindows.org](https://gitforwindows.org/) and press Download.

* On the download page, click on either the 32-bit or 64-bit version depending on your Windows system. 
...
* Please use the default path, i.e. `C:/Program Files`, which makes it easier for RStudio to find Git. 

* During Git installation, select (a) Use Git from Windows Command Prompt, and (b) Enable Git Credential Manager. 
...
- Suggest you check the box “Open in new session”.
...
Now just … repeat. Do work somewhere. Commit it. Push it or pull it depending on where you did it, but get local and remote “synced up”.
Repeat.
...
We have do this before and now repeat the steps.
...
- If you use the [usethis](https://cran.r-project.org/web/packages/usethis/index.html) package, set the existing directory as the current project with `proj_set()`, then do `usethis::use_rstudio()`.
...
- If you have the [usethis](https://cran.r-project.org/web/packages/usethis/index.html) package, call `usethis::use_git()`.
...
If you use the [usethis](https://cran.r-project.org/web/packages/usethis/index.html) package AND you have configured a GitHub Personal Access Token (PAT), do this in
