...
Inline code such as `x = x+1` can be created by wrapping code with a backstick sign ` ` `, e.g. `r inline_mdcode("x = x+1")`.  
...
Let us **display** 1+1. Using inline Markdown code `r inline_mdcode("1+1")` gives `1+1` and Markdown code block gives
...
Let us **compute** 1+1. Using inline R code `r inline_rcode("1+1")` gives `r 1+1` and R code block gives
...
Placing an `r` **immediately** after a backstick sign ` ` ` starts inline R code, e.g. `r inline_rcode("some_r_code")`, that will always be evaluated, showing only result but not code. 
...
```
The iris dataset has `r inline_rcode_inblock("nrow(diamonds)")` rows.
...
Placing an `{r}` **immediately** after three backstick signs ` ``` ` with another set of three closing backstick signs ` ``` ` creates an R code block, e.g. `r inline_rchunk("lines of code")`. By default, the code block will be evaluated, showing the code and the result. 
...
`r inline_mdcode("TODO")`

* B: inline_mdcode_inblock("TODO")

```
`r inline_mdcode_inblock("TODO")`
...
```
`r inline_rcode_inblock("1+2")`

```

* E: inline_header("fig.width=5, collapse=TRUE")

`r inline_header("fig.width=5, collapse=TRUE")`

* F: inline_rchunk("string")

`r inline_rchunk("x=1; y=x+1")`
...
|Code|Effect|Note|
|---|---|---|
| `r inline_mdcode("TODO")` | `TODO` | raw Markdown code |
| `r inline_rcode('highlight("todo")')` | `r highlight("todo")` | HTML code |
| `r inline_rcode('highlight_css("todo")')` | `r highlight_css("todo")` | CSS code |
| `r inline_rcode('text_spec("TODO", color="red", background="yellow")')` | `r text_spec("TODO", color="red", background="yellow")` | `kableExtra::text_spec()` |
...
```
`r inline_rcode_inblock("toggle_button('id2')")`
...
|`r emo::ji("blush")`|:blush:||`r emo::ji("sob")`|:sob:|
|`r emo::ji("sunglasses")`|:sunglasses:||`r emo::ji("scream")`|:scream:|
|`r emo::ji("exclamation")`|:exclamation:||`r emo::ji("question")`|:question:|
|`r emo::ji("+1")`|:+1:||`r emo::ji("-1")`|:-1:|
...
```{r collapse=TRUE, cache=TRUE, fig.show="hide"}
library(gridSVG)
p
...
```{r fig.width=4, fig.height=4, fig.cap="caption: a graph shrunk in size"}
plot(cars)
```
...
```{r fig.show="hold", fig.width=4, fig.height=4, results="hold"}
plot(cars)
3+3
2+1
```
...
A chunk can be given an optional name, e.g. `r inline_header("chunkname")`, which offers three advantages. 
...
```{r results="asis", echo=FALSE}
cat(paste(colnames(cars), collapse=" | "), "--- | ---", sep="\n")
cat(apply(cars[1:5,], 1, function(X) paste(X, collapse=" | ")), sep = "\n")
```
